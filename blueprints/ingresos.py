...
def list_ingresos():
    ...
    return render_template('ingresos/list.html',
        ingresos=ingresos,
        locales=locales,
...
def nuevo_ingreso():
    ...
    if request.method == 'POST':
        ...
        for codigo, _ in MEDIOS_COBRO:
            ...
            try:
                val = float(val_str.replace('.', '').replace(',', '.') if ',' in val_str else val_str)
            except (ValueError, AttributeError):
                val = 0
            if val < 0:
        ...
        if not errors and total > 0:
            if abs(suma_medios - total) > 0.01:
                errors.append(f'La suma de los medios de cobro (${suma_medios:,.2f}) debe coincidir con el total (${total:,.2f}).')

        if errors:
            for e in errors:
                flash(e, 'danger')
            db.close()
            return render_template('ingresos/form.html',
                locales=locales, medios_cobro=MEDIOS_COBRO,
                ingreso=request.form, medios_vals=medios_data, modo='nuevo')
        ...
        for codigo, _ in MEDIOS_COBRO:
            monto_medio = medios_data.get(codigo, 0)
            if monto_medio > 0:
                db.execute(
...
def editar_ingreso(ingreso_id):
    ...
    if request.method == 'POST':
        ...
        if errors:
            ...
            return render_template('ingresos/form.html',
                locales=locales, medios_cobro=MEDIOS_COBRO,
                ingreso=request.form, medios_vals=medios_data,
                modo='editar', ingreso_id=ingreso_id)

        db.execute(
            '''UPDATE ingresos SET fecha_desde=?, fecha_hasta=?, local_id=?, total=?,
               observaciones=?, updated_at=CURRENT_TIMESTAMP WHERE id=?''',
    ...
    return render_template('ingresos/form.html',
        locales=locales, medios_cobro=MEDIOS_COBRO,
        ingreso=ingreso, medios_vals=medios_vals,
        modo='editar', ingreso_id=ingreso_id)
...
def detalle_ingreso(ingreso_id):
    ...
    db.close()
    return render_template('ingresos/detalle.html',
        ingreso=ingreso, medios=medios, medios_cobro=MEDIOS_COBRO)
